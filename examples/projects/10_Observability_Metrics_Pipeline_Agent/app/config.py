"""Scenario configuration for this example."""

EXAMPLE_NUMBER = 10
EXAMPLE_TITLE = "Observability Metrics Pipeline Agent"
EXAMPLE_SUMMARY = "Combine policy and telemetry to build metrics-first operational reporting."
DOMAIN = "observability engineering"
MODEL = "ollama_chat/gpt-oss:20b"

# Tier controls architecture complexity.
# 1: sync + tools
# 2: + streaming + thread continuity
# 3: + policy + telemetry projection
# 4: + sqlite memory + batch orchestration
# 5: + governance + portfolio aggregation
TIER = 3

DEFAULT_PROMPT = (
    "Run observability engineering analysis with concrete recommendations and operational next steps."
)
